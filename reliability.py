...
def parse_rss_feed(xml: str, author: str) -> list[dict]:
    ...
    out = []
    for it in items:
        title = (it.find("title") or {}).get_text(strip=True)
        link  = (it.find("link")  or {}).get_text(strip=True)
        pub   = (it.find("pubDate") or it.find("published") or {}).get_text(strip=True)
        desc  = (it.find("description") or it.find("summary") or {}).get_text(strip=True)
        # filter by author name in creator field if present
        creator = (it.find("dc:creator") or it.find("author") or {}).get_text(strip=True)
